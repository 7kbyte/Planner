...
import type { RepeatType, RepeatTemplateFormData, RepeatTemplate } from '../types/task'
import { REPEAT_CONFIG } from '../types/task'
...
export default function AddRepeatModal({ isOpen, onClose, onSave, template }: Props) {
  ...
  return (
    <div ref={overlayRef} onClick={(e) => { if (e.target === overlayRef.current) onClose() }}
      ...
      <div className="relative w-full max-w-md mx-4 bg-white dark:bg-gray-800 rounded-2xl shadow-xl ring-1 ring-gray-200/60 dark:ring-gray-700/60 p-6"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {(['low', 'medium', 'high'] as const).map((p) => (
                <button key={p} type="button" onClick={() => setPriority(p)} className={`flex-1 py-2 rounded-xl text-sm font-medium transition-all ${priority === p ? (p === 'high' ? 'bg-red-500 text-white shadow-sm' : p === 'medium' ? 'bg-amber-500 text-white shadow-sm' : 'bg-green-500 text-white shadow-sm') : 'bg-gray-100 dark:bg-gray-700 text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">重复频率</label>
            <div className="flex gap-2">
              {(['daily', 'weekly', 'monthly'] as const).map((r) => (
                <button key={r} type="button" onClick={() => setRepeat(r)} className={`flex-1 py-2 rounded-xl text-sm font-medium transition-all ${repeat === r ? 'bg-indigo-500 text-white shadow-sm' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>
                  {REPEAT_CONFIG[r].icon} {REPEAT_CONFIG[r].label}
                </button>
              ))}
            </div>
          </div>
